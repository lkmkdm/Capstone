...
public class FirebaseDataToUI : MonoBehaviour
{
    ...
    {
        db.Collection("users").Document("testuser@example.com").Collection("ability").Document("current").GetSnapshotAsync().ContinueWithOnMainThread(task =>
        {
            if (task.IsCompleted && task.Result.Exists)
            {
                ...
                StartCoroutine(AnimateText(processingSpeedText, "처리속도", GetValueFromDict(data, "processingspeed")));
